...
public class RoundDown extends MultiArgMethod {
    ...
    public void checkArg(List<Var> argList) throws ExpressionException {
        if (argList.size() > 2) {
            throw new ExpressionException(ErrorCode.GRAMMAR_ERROR, "RoundDown函数只接受两个数值参数");
        }
        if (!argList.get(0).valueType().accept(ValueType.NUMBER)) {
            throw new ExpressionException(ErrorCode.GRAMMAR_ERROR, "RoundDown函数第一个参数必须是数值");
        }
        if (argList.size() == 2) {
            if (argList.get(1).valueType() != ValueType.NUMBER) {
                throw new ExpressionException(ErrorCode.GRAMMAR_ERROR, "RoundDown函数第二个参数必须是数值");
            }
        }
    }
    ...
    public Value<?> calculate(BrokersExpress node, ParamCollection param) {
        ...
        if (args.size() == 1) {
            return Value.of(args.get(0).execute(param).toMath().downScale());
        } else {
            return Value.of(args.get(0).execute(param).toMath().downScale(args.get(1).execute(param).toInt()));
        }
    }
