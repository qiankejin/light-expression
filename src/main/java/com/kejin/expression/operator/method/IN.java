...
public class IN extends MultiArgMethod {

    @Override
    public void checkArg(List<Var> argList) throws ExpressionException {
        if (argList.size() < 2) {
            throw new ExpressionException(ErrorCode.GRAMMAR_ERROR, "in函数参数个数必须大于2");
        }
        ValueType type = null;
        for (Var express : argList) {
            if (type == null) {
                ...
                throw new ExpressionException(ErrorCode.GRAMMAR_ERROR, "in函数参数必须是相同类型");
            }
        }
    }
    ...
    @Override
    public BooleanValue calculate(BrokersExpress node, ParamCollection param) {
