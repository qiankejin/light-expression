...
public class Sin extends SingleArgMethod {
    ...
    protected void checkArg(Var arg) {
        if (!arg.valueType().accept(ValueType.NUMBER)) {
            throw new ExecuteException(ErrorCode.GRAMMAR_ERROR, "sin函数入参必须是数值");
        }
    }
    ...
    protected DoubleValue methodCalculate(Var arg, ParamCollection param) {
        return Value.of(Decimal.of(Math.sin(arg.execute(param).toMath().multiply(Math.PI).divide(180).toDouble())));
    }
