...
public class Cos extends SingleArgMethod {
    ...
    protected void checkArg(Var arg) throws ExpressionException {
        if (!arg.valueType().accept(ValueType.NUMBER)) {
            throw new ExpressionException(ErrorCode.GRAMMAR_ERROR, "cos函数入参必须是数值");
        }
    }

    @Override
    protected DoubleValue methodCalculate(Var arg, ParamCollection param) {
        return Value.of(Decimal.of(Math.cos(arg.execute(param).toMath().multiply(Math.PI).divide(180).toDouble())));
    }
